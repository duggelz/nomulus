...
def domain_registry_repositories(**kw_args):
  """Imports dependencies for Nomulus."""
  domain_registry_bazel_check()

  # Check for misspellings in kw_args
  dependency_names = {d['name']: d for d in dependencies}
  for key, value in kw_args.items():
    if not key.startswith("omit_"):
      fail('Parameter "%s" does not start with "omit_"' % key)
    name = key[5:]
    if not name in dependency_names:
      fail('Parameter "%s" references dependency "%s" which is not in dependency list: %s' % (
          key, name, dependency_names.keys()))
    if value not in [True, False]:
      fail('Parameter "%s" has invalid value %r, should be True or False' %
           (key, value))

  # Load all dependencies except those that should be omitted
  for dependency in dependencies:
    dependency_name = dependency['name']
    omit = kw_args.get('omit_' + dependency_name, False)
    if not omit:
      java_import_external(**dependency)
